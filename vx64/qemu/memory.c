...
{
    free(container_of(iorange, MemoryRegionIORange, iorange));
}
...
void memory_region_add_coalescing(MemoryRegion *mr,
                                  ...
                                  uint64_t size)
{
    CoalescedMemoryRange *cmr = malloc(sizeof(*cmr));
...
void address_space_init(AddressSpace *as, MemoryRegion *root)
{
    memory_region_transaction_begin();
    as->root = root;
    as->current_map = malloc(sizeof(FlatView));
                           ...
                           MemoryRegionListHead *alias_print_queue)
{
    ...
    if (mr->alias) {
        ...
        if (!found) {
            ml = malloc(sizeof(MemoryRegionList));
            ml->mr = mr->alias;
            ml->printed = false;
    ...
    QTAILQ_FOREACH(submr, &mr->subregions, subregions_link) {
        new_ml = malloc(sizeof(MemoryRegionList));
        new_ml->mr = submr;
