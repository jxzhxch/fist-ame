...
#else
# define ALLOC(P, SIZE) \
    do { P = malloc(SIZE); memset(P, 0, SIZE);} while (0)
#endif
...
static inline void *alloc_code_gen_buffer(void)
{
    void *buf = malloc(code_gen_buffer_size);
...
static inline void code_gen_alloc(size_t tb_size)
{
    ...
    tbs = malloc(code_gen_max_blocks * sizeof(TranslationBlock));
}
                                   ...
                                   int is_cpu_write_access)
{
    ...
    while (tb != NULL) {
        ...
        if (!(tb_end <= start || tb_start >= end)) {
            ...
            /* modify by fshooter */
            /* 原来是测试current_tb->cflags来确保当前块不是因为自修改而临时生成的块，
             * 但某些情况下，一个指令单独翻译成了一个块但这个指令又自修改自身的时候，这时候其cflags=0所以系统会重新翻译该指令
             * 但在实际cpu上，cpu实际把当前指令缓存了，当前指令修改自身不会触发真正的cpu去重新执行新指令，恶心啊
             * 所以修改为测试块的指令数，如果一个块只有一条指令那它不会触发自修改
             * 但是如果修改为测试icount的话当前块会被invalidate掉，然后会不会极端条件下被回收导致内存不可用呢？？？FIXME */
            /*
            if (current_tb == tb &&
                (current_tb->cflags & CF_COUNT_MASK) != 1) { */
            if (current_tb == tb) {
                  if(current_tb->icount != 1) {
					...
					cpu_get_tb_cpu_state(env, &current_pc, &current_cs_base,
                	  ...
                	  /* 把current_tb设置成空因为接下来的那句tlb_phy_invalidate就要把他销毁了 */
                	  current_tb = NULL;
                  }
            }
#endif /* TARGET_HAS_PRECISE_SMC */
