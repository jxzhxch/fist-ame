...
static void phys_map_node_reserve(unsigned nodes)
{
    if (phys_map_nodes_nb + nodes > phys_map_nodes_nb_alloc) {
        ...
        phys_map_nodes = (Node*)realloc(phys_map_nodes,
        		sizeof(Node) * phys_map_nodes_nb_alloc);
    }
}
...
static uint16_t phys_section_add(MemoryRegionSection *section)
{
    if (phys_sections_nb == phys_sections_nb_alloc) {
        ...
        phys_sections = (MemoryRegionSection*)realloc(phys_sections,
                                sizeof(MemoryRegionSection) * phys_sections_nb_alloc);
    }
...
static subpage_t *subpage_init(hwaddr base)
{
    ...
    mmio = malloc(sizeof(subpage_t)); memset(mmio, 0, sizeof(subpage_t));
...
{
    AddressSpaceDispatch *d = (AddressSpaceDispatch*)malloc(sizeof(AddressSpaceDispatch));
...
void invalidate_and_set_dirty(hwaddr addr,
                                     hwaddr length)
{
                        ...
                        bool is_write)
{
    ...
    while (len > 0) {
        ...
        if (!(memory_region_is_ram(section->mr) && !section->readonly)) {
            ...
            bounce.buffer = malloc(TARGET_PAGE_SIZE);
            bounce.addr = addr;
            bounce.len = l;
            if (!is_write) {
