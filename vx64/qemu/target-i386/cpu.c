...
static char *x86_cpuid_get_vendor(Object *obj, Error **errp)
{
    ...
    value = (char *)malloc(CPUID_VENDOR_SZ + 1);
...
{
	// patch by fshooter, remove glib dependence, maybe error!
    CPUX86State *env = &cpu->env;
    x86_def_t def1, *def = &def1;
    Error *error = NULL;
    char name[MAX_PATH];
    char features[MAX_PATH];
    const char* split = strchr(cpu_model, ',');
    if(!split) {
    	split = cpu_model + strlen(cpu_model);
    }

    memset(def, 0, sizeof(*def));

    memcpy(name, cpu_model, split - cpu_model);
    name[split - cpu_model] = 0;
    if(*split) {
    	strcpy(features, split + 1);
    } else {
    	strcpy(features, "");
    }
                   ...
                   uint32_t *ecx, uint32_t *edx)
{
    ...
    case 1:
        *eax = env->cpuid_version;
        /* fixme: 没有按照原有qemu的要求设置cpuid_apic_id， 这里可以选择返回一个假的cpuid */
