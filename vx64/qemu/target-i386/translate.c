 ...
 reswitch:
    ...
    case 0xf7:
        ...
        case 6: /* div */
            switch(ot) {
            case OT_BYTE:
            	// modifyed by fshooter
            	// div 0时会抛出异常，而s->cc_op只在翻译时存在，因此，将s->cc_op保存到cpu_context中
            	// 以便于在发生div 0异常时能够恢复eflags
            	if (s->cc_op != CC_OP_DYNAMIC) {
            	    gen_op_set_cc_op(s->cc_op);
            	}
                gen_jmp_im(pc_start - s->cs_base);
...
#if 1
        gen_interrupt(s, EXCP01_DB, pc_start - s->cs_base, s->pc - s->cs_base);
        // patch by fshooter, 应该生成1号中断，gen_debug会造成生成调试异常(EXCP_DEBUG)
